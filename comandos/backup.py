...
class PokemonComando(commands.Cog):
    ...
    async def capturar_pokemon(self, ctx):
        ...
        raridades = ['Common', 'Uncommon', 'Rare', 'Very Rare', 'Legendary', 'Mythical']
        probabilidades = {
            'Common': 50,
            'Uncommon': 60,
            'Rare': 70,
        ...
        # Escolhendo uma raridade aleatória
        raridade = random.choices(raridades, weights=[50, 60, 70, 80, 90, 100], k=1)[0]
    ...
    async def meupokemon(self, ctx):
        ...
        cursor = self.db_connection.cursor()
        cursor.execute("SELECT pokemon_nome, raridade, ataque, tipos FROM pokemons_capturados WHERE nome_usuario = %s",
                       (ctx.author.name,))
